<template>
    <div>
        <div class="link text-center mb-100" v-if="URI !== '/customer-orders'">
            <a href="/customer-orders" class="btn btn-success btn-lg"
                ><h1>Go to Orders Page</h1></a
            >
        </div>
        <div
            class="alert alert-warning text-center mb-100"
            v-if="URI === '/customer-orders'"
        >
                    ...
                    <tr v-for="option in optiongroup.options" class="text-info">
                        ...
                        <td>
                            <button
                                ...
                                @click.prevent="deleteResource(option.id)"
                            >
                                Delete
                            </button>
                        </td>
                    ...
                    >
                        ...
                        <td>
                            <button
                                class="btn btn-danger btn-sm"
                                @click.prevent="
                                    deleteResource(product.sales.id)
                                "
                            >
                                Delete
                            </button>
                ...
                <tbody>
                    <tr class="text-info" v-for="item in data">
                        ...
                        <td
                            v-if="URI === '/customer-orders'"
                            :class="
                                classes(item.hiddenOrder || item.status.name)
                            "
                        >
                            <h4 class="text-info">
                        ...
                        <td v-if="URI === '/customer-orders'">
                            ...
                            <p v-if="item.order_type === 'Pick-up'">
                                at {{ item.pickup_time }}
                            </p>
                        </td>
                        <td>
                            <button
                                ...
                                @click.prevent="showResource(item.id)"
                            >
                                Show
                            </button>
                            <button
                                ...
                                @click.prevent="email(item.email)"
                            >
                                Email
                            </button>
                            <button
                                v-else
                                class="btn btn-danger btn-sm"
                                @click.prevent="deleteResource(item.id)"
                            >
                                {{ deleteBtnMethod }}
                            </button>
                        </td>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            deleteBtnMethod() {
                return window.location.pathname === '/customer-orders'
                    ? 'Mark As Processed'
                    : 'Delete';
            },
        },
