            ...
            @foreach( $orders as $order )
                @if(!$order->hiddenOrder())
                    ...
                    <li class="list-group-item hideable">
                        <a href="/order/{{ $order->id }}" class="admin-links"><h4 class="admin-links">Order: {{ $order->id }}</h4>
                        ...
                        <p class="text-right"><a href="/order/{{ $order->id }}" class="btn btn-primary btn-sm">View order</a></p>
                        <form action="/hide-orders/{{ $order }}" method="POST">
                            {{ csrf_field() }}
                    ...
                    @else
                        ...
                        <form action="/show-order/{{ $order }}" method="POST">
                            {{ csrf_field() }}
                            <input type="hidden" value="{{ $order->id }}" name="id">
                            <input type="hidden" value="{{ $order->name }}" name="name">
                            <input type="hidden" value="{{ $order->address }}" name="address">
