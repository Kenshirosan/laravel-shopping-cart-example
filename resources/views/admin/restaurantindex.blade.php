                ...
                <ul class=list-group>
                    @if(Auth::user()->theboss)
                        ...
                        @foreach( $orders as $order )
                            <li class="list-group-item"><h4>Latest Orders: {{ $order->id }}</h4>{{ $order->name }} {{ $order->last_name }} paid $<strong>{{ $order->price /100 }}</strong> for {{ $order->items }} on <strong>{{ $order->created_at->toFormattedDateString() }}</strong> at {{    $order->created_at->toTimeString() }}</li>
                        @endforeach
                    @endif
