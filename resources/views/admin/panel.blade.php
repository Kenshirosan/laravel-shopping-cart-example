        ...
        <ul class=list-group>
            @if(Auth::user()->theboss)
                ...
                @foreach($yearlyTotal as $total)
                    <li class="list-group-item text-right">Total for <strong class="text text-info">{{ $total->year }}: ${{ $total->total /100 }}</strong></li>
                @endforeach
                @foreach ($totalOrders as $monthlyTotal)

                    <li class="list-group-item text-right">
                        <h4> Total for {{ $monthlyTotal->month }}:</h4>
                        <p>Total orders including taxes for {{ $monthlyTotal->month }} : ${{ $monthlyTotal->total /100}}</p>
                    </li>
                @endforeach
